...
public:
    ...
    Frame get_builtin_decls() { return global.frame.get_parent(); }
    Declarations& get_system_declarations() { return system_declarations; }
    ...
    const PositionIndex::Line& find_first_position(uint32_t position) const;

    Variable* add_variable_to_function(Function*, Frame, Type, std::string_view, Expression initial, position_t);
    Variable* add_variable(Declarations*, Frame frame, Type type, std::string_view name, Expression initial, position_t);
...
protected:
    ...
    // Global declarations
    Declarations global;
    Declarations system_declarations;
